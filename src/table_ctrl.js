...
import * as form from './addOperation'
...
export class TableCtrl extends MetricsPanelCtrl {
  constructor ($scope, $injector, templateSrv, annotationsSrv, $sanitize, variableSrv) {
    ...
    $(document).on('click', 'tr.tr-affect#master-data-operation-tr', function (e) {
      ...
      if (!~nameIndex) {
        utils.alert('error', 'Error', 'Get not get this operation from the database, please contact the dev team')
      } else {
        $scope.ctrl.currentOperation = utils.findOperationByName($scope.ctrl.operations, rowData[nameIndex])[0]
        operationOption.showOptionModal($scope.ctrl)
      }
    })
  }
  ...
  onDataReceived (dataList) {
    ...
    this.operations = utils.getRestructuredData(dataList[0].columns, dataList[0].rows)
    this.operationDimensions = dataList[0].columns.map(col => col.text)
    this.operationNames = this.operations.map(operation => operation.name)
